...
- Preserve that custom panel on the desktop. `windows/settings-window.cs` hosts the same HTML above desktop shortcuts, opens with normal foreground behavior, and exits on close. `grid-native-settings.js` bridges validated changes to Lively; acknowledge saves only after native persistence is confirmed. Do not replace the panel with Lively's generated editor.
